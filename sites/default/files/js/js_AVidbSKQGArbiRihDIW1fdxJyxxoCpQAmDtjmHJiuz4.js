...
(function(Drupal, drupalSettings, drupalTranslations, console, Proxy, Reflect) {
    ...
    Drupal.t = function(str, args, options) {
        ...
        if (drupalTranslations ? .strings ? .[options.context] ? .[str]) str = drupalTranslations.strings[options.context][str];
    ...
    Drupal.formatPlural = function(count, singular, plural, args, options) {
        ...
        let index = 0;
        if (drupalTranslations ? .pluralFormula) index = count in drupalTranslations.pluralFormula ? drupalTranslations.pluralFormula[count] : drupalTranslations.pluralFormula.default;
        else {
    ...
    Drupal.deprecationError = ({
        ...
        if (drupalSettings.suppressDeprecationErrors === false && console ? .warn) console.warn(`[Deprecation] ${message}`);
    };
...
;
/* @license GPL-2.0-or-later https://www.drupal.org/licensing/faq */
Drupal.behaviors.arizonaeduChangeNewsButtonColor = {
    attach: function(context) {
        let el = context.querySelector(".view-az-news .view-footer a");
        el.classList.remove("btn-outline-blue");
        el.classList.add("btn-red");
        el.textContent = "All News Stories";
        el.setAttribute("title", "All News Stories");
    }
};;